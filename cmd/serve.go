...
func init() {
	...
	serverCmd.Flags().StringVarP(&filePath, "file-path", "f", "", "base directory for the file-reading functions and served files, which exist only when it is given")
	serverCmd.Flags().StringVarP(&writePath, "write-path", "w", "", "base directory for file write functions; must be under --file-path")
	serverCmd.Flags().BoolVar(&allowKill, "allow-kill", false, "enable the kill function")
	...
	enablesFeature(serverCmd, "file-path", "readfiles")
	enablesFeature(serverCmd, "write-path", "writefiles")
	enablesFeature(serverCmd, "allow-kill", "allowkill")
