...
// MQTTClientDefinition holds the decoded HCL for a client "mqtt" block.
...
// MQTTAuthDefinition is the optional auth block inside a client "mqtt" block.
...
// MQTTWillDefinition is the optional will block inside a client "mqtt" block.
...
// MQTTPublisherDef is one publisher block inside a client "mqtt" block.
...
// builtMQTTPublisherSpec holds what is known at config time about one publisher,
// ready to build an MQTTPublisher in Start().
...
// builtMQTTSubscriberSpec holds what is known at config time about one subscriber,
// ready to build an MQTTSubscriber in Start().
...
// by dispatching OnEvent to all publishers. Implements config.Startable and config.Stoppable.
...
// When publishers are present:
//   - "publishers" — a subscriber capsule wrapping this wrapper (fan-out to all publishers)
//   - "publisher"  — an object mapping each publisher name to its own subscriber capsule
//
// When no publishers are configured, returns a plain ClientCapsule.
...
// Start builds all publishers and subscribers, connects to the MQTT broker,
// and blocks until the first connection is established.
...
// Stop sends a graceful DISCONNECT and waits for the connection manager to shut down.
...
// ProcessMQTTClientBlock decodes and validates a client "mqtt" block and
// returns an MQTTClientWrapper ready to be started.
func ProcessMQTTClientBlock(config *Config, block *hcl.Block, remainingBody hcl.Body) (Client, hcl.Diagnostics) {
	...
	// Validate unique publisher names.
	...
	// Validate unique subscriber names.
	...
	if def.TLS != nil && def.TLS.Enabled {
		cfg, tlsErr := def.TLS.BuildTLSClientConfig(config.BaseDir)
		if tlsErr != nil {
			return nil, hcl.Diagnostics{{
				Severity: hcl.DiagError,
	...
	if IsExpressionProvided(def.KeepAlive) {
	...
	if IsExpressionProvided(def.SessionExpiryInterval) {
	...
	if def.Auth != nil {
		...
		if IsExpressionProvided(def.Auth.Password) {
	...
	// ── Reconnect backoff function ─────────────────────────────────────────

	reconnectFn, reconnDiags := buildMQTTReconnectBackoffFunc(config, def.Reconnect)
	...
	onConnect := makeMQTTLifecycleHook(config, def.OnConnect)
	onDisconnect := makeMQTTLifecycleHook(config, def.OnDisconnect)
	...
	pubSpecs, pubDiags := buildMQTTPublisherSpecs(config, def.Publishers)
	...
	subSpecs, subDiags := buildMQTTSubscriberSpecs(config, clientName, def.Subscribers)
...
func buildMQTTPublisherSpecs(config *Config, defs []MQTTPublisherDef) ([]builtMQTTPublisherSpec, hcl.Diagnostics) {
	specs := make([]builtMQTTPublisherSpec, 0, len(defs))
	for _, def := range defs {
		spec, diags := buildMQTTPublisherSpec(config, def)
		if diags.HasErrors() {
			return nil, diags
		}
...
func buildMQTTPublisherSpec(config *Config, def MQTTPublisherDef) (builtMQTTPublisherSpec, hcl.Diagnostics) {
	spec := builtMQTTPublisherSpec{name: def.Name}

	// Publisher-level QoS default (default: 1 per spec).
	spec.defaultQoS = 1
	if def.QoS != nil {
	...
	for _, tmDef := range def.TopicMappings {
		...
		if IsExpressionProvided(tmDef.MQTTTopic) {
...
func buildMQTTSubscriberSpecs(config *Config, clientName string, defs []MQTTSubscriberDef) ([]builtMQTTSubscriberSpec, hcl.Diagnostics) {
	specs := make([]builtMQTTSubscriberSpec, 0, len(defs))
	for _, def := range defs {
		spec, diags := buildMQTTSubscriberSpec(config, clientName, def)
		if diags.HasErrors() {
			return nil, diags
		}
		specs = append(specs, spec)
...
func buildMQTTSubscriberSpec(config *Config, clientName string, def MQTTSubscriberDef) (builtMQTTSubscriberSpec, hcl.Diagnostics) {
	spec := builtMQTTSubscriberSpec{
		name:           def.Name,
		handleRetained: true, // default per spec
	...
	if hasSubscriber {
		sub, diags := GetSubscriberFromExpression(config, def.Subscriber)
		if diags.HasErrors() {
			return spec, diags
		}
		spec.subscriber = sub
	} else {
		spec.subscriber = &ActionSubscriber{
			Config:     config,
	...
	for _, tsDef := range def.Subscriptions {
		...
		if IsExpressionProvided(tsDef.VinculumTopic) {
...
// makeMQTTTopicFunc returns an MQTTTopicFunc that evaluates expr per-message
// with topic, msg, and fields in scope.
func makeMQTTTopicFunc(cfg *Config, expr hcl.Expression) mqttpublisher.MQTTTopicFunc {
...
// makeMQTTVinculumTopicFunc returns a VinculumTopicFunc that evaluates expr
// per-message with ctx.topic, msg, and fields in scope. An empty string return
// value signals "use the MQTT topic verbatim" to the subscriber.
func makeMQTTVinculumTopicFunc(cfg *Config, expr hcl.Expression) mqttsubscriber.VinculumTopicFunc {
	return func(mqttTopic string, fields map[string]string, msg any) (string, error) {
		...
		// ctx.topic is the MQTT topic of the incoming message.
		ctxBuilder := NewContext(context.Background()).
...
// makeMQTTLifecycleHook returns a func(context.Context) that evaluates expr
// when called (for on_connect / on_disconnect hooks). Returns nil if not provided.
func makeMQTTLifecycleHook(cfg *Config, expr hcl.Expression) func(ctx context.Context) {
	if !IsExpressionProvided(expr) {
		return nil
	}
	return func(ctx context.Context) {
		evalCtx, diags := NewContext(ctx).BuildEvalContext(cfg.evalCtx)
		if diags.HasErrors() {
			cfg.Logger.Error("mqtt lifecycle hook: build eval context", zap.Error(diags))
			return
		}
		_, diags = expr.Value(evalCtx)
		if diags.HasErrors() {
			cfg.Logger.Error("mqtt lifecycle hook: eval failed", zap.Error(diags))
		}
	}
}

// buildMQTTReconnectBackoffFunc builds a ReconnectBackoffFunc from a ReconnectDefinition.
// Returns nil (use autopaho's default backoff) when def is nil.
func buildMQTTReconnectBackoffFunc(cfg *Config, def *ReconnectDefinition) (func(int) time.Duration, hcl.Diagnostics) {
	if def == nil {
		return nil, nil
	}
	...
	if IsExpressionProvided(def.InitialDelay) {
		d, diags := cfg.ParseDuration(def.InitialDelay)
		if diags.HasErrors() {
			return nil, diags
		}
		initialDelay = d
	...
	if IsExpressionProvided(def.MaxDelay) {
		d, diags := cfg.ParseDuration(def.MaxDelay)
		if diags.HasErrors() {
			return nil, diags
		}
		maxDelay = d
