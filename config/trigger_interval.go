...
func (t *IntervalTrigger) run() {
	...
	for {
		...
		if runCount == 0 && IsExpressionProvided(t.initialDelayExpr) {
			delayExpr = t.initialDelayExpr
		} else if lastErr != nil && IsExpressionProvided(t.errorDelayExpr) {
		...
		dur, err := parseDurationFromValue(delayVal)
		if err != nil {
		...
		if IsExpressionProvided(t.stopWhenExpr) {
...
func processIntervalTrigger(config *Config, block *hcl.Block, triggerDef *TriggerDefinition) hcl.Diagnostics {
