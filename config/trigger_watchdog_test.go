...
func TestTriggerWatchdog(t *testing.T) {
	cfg, diags := NewConfig().WithSources(triggerWatchdogVCL).WithLogger(testLogger(t)).Build()
	require.False(t, diags.HasErrors(), "unexpected diagnostics: %v", diags)

	require.Contains(t, cfg.CtyTriggerMap, "heartbeat")
	val := cfg.CtyTriggerMap["heartbeat"]
	...
	assert.Contains(t, cfg.TriggerDefRanges, "heartbeat")
}

func TestTriggerWatchdogFull(t *testing.T) {
	cfg, diags := NewConfig().WithSources(triggerWatchdogFullVCL).WithLogger(testLogger(t)).Build()
	require.False(t, diags.HasErrors(), "unexpected diagnostics: %v", diags)

	wdog, err := GetWatchdogTriggerFromCapsule(cfg.CtyTriggerMap["full"])
	require.NoError(t, err)
...
func TestTriggerWatchdogStopBeforeFire(t *testing.T) {
	...
	wdog, err := GetWatchdogTriggerFromCapsule(cfg.CtyTriggerMap["heartbeat"])
	require.NoError(t, err)
...
`)
	cfg, diags := NewConfig().WithSources(src).WithLogger(testLogger(t)).Build()
	require.False(t, diags.HasErrors(), "unexpected diagnostics: %v", diags)

	wdog, err := GetWatchdogTriggerFromCapsule(cfg.CtyTriggerMap["fast"])
	require.NoError(t, err)
...
func TestSetNoValueSetsNull(t *testing.T) {
	// set(var.x) with no value sets the variable to null.
	src := []byte(`
var "x" { value = 42 }
