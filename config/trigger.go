...
func (h *TriggerBlockHandler) GetBlockDependencyId(block *hcl.Block) (string, hcl.Diagnostics) {
	if len(block.Labels) == 2 && (block.Labels[0] == "after" || block.Labels[0] == "interval" || block.Labels[0] == "once" || block.Labels[0] == "start" || block.Labels[0] == "watchdog") {
...
func (h *TriggerBlockHandler) Process(config *Config, block *hcl.Block) hcl.Diagnostics {
	...
	case "watchdog":
		return processWatchdogTrigger(config, block, &triggerDef)
	default:
		return hcl.Diagnostics{&hcl.Diagnostic{
			Severity: hcl.DiagError,
			Summary:  "Invalid trigger type",
			Detail:   fmt.Sprintf("Invalid trigger type: %q. Valid types are: after, cron, interval, once, shutdown, signals, start, watchdog", block.Labels[0]),
