...
`)
	...
	assert.Contains(t, config.CtyClientMap, "gpt")
	clients, ok := config.Clients["openai"]
	require.True(t, ok)
	c, ok := clients["gpt"]
	require.True(t, ok)
...
`)
	...
	oai := config.Clients["openai"]["gpt"].(*OpenAIClient)
...
`)
	_, diags := cfg.NewConfig().WithSources(vcl).WithLogger(newTestLogger(t)).Build()
