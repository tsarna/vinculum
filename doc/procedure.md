...
One of these limitations is that HCL does not allow a variable to be reassigned in a block. You can assign a value and then overwrite it in a conditional or loop block, but not within the same block.

Another is that there is no syntax for evaluating an expression for side effects. Every statement must be either an assignent or a block. See  the section on Discard Names below for how to deal with this.
